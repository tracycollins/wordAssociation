...
function ViewTreepack() {
  ...
  //   this.circle = nodeSvgGroup
  //     .append("circle")
  //     .attr("id", nodePoolId)
  //     // .attr("nodeId", nodePoolId)
  ...
  //   this.label = nodeLabelSvgGroup.append("text")
  //     .attr("id", nodePoolId + "_label")
  //     .attr("nodeId", "")
  ...
  //   this.topTermLabel = nodeTopTermLabelSvgGroup.append("text")
  //     .attr("id", nodePoolId + "_labelTopTerm")
  //     .attr("nodeId", "")
  ...
  function Node(nodePoolId){
    ...
    this.circle = nodeSvgGroup.append("circle")
      .attr("id", nodePoolId)
      // .attr("nodeId", nodePoolId)
      .attr("r", 1e-6) 
      .attr("cx", 0.5*width)
  ...
  function resetNode(n, callback){
    // console.log("resetNode n | " + n.nodePoolId + " | " + n.nodeId);
    n.age = 1e-6;
    n.ageMaxRatio = 1e-6;
  ...
  var ageNodes = function (callback) {
    ...
    nodeIdArray.forEach(function(nodeId){
      ...
      if (removeDeadNodesFlag && (age >= nodeMaxAge)) {
        ...
        // localNodeHashMap.remove(node.nodePoolId);

        // if (node.isValid) {
          // node.isDeadValid = false;
          resetNode(node, function(n){
            // localNodeHashMap.remove(n.nodePoolId);
            nodePool.recycle(n);        
  ...
  var updateTopTerm = function(callback) {
    ...
    nodeTopTermLabels = nodeTopTermLabelSvgGroup.selectAll("text")
      .data(nodesTopTerm.filter(function(d){ return d.isValid ? this : null; }));
  ...
  var updateNodeCircles = function(callback) {
    ...
    nodeCircles = nodeSvgGroup.selectAll("circle")
      .data(nodeArray.filter(function(d){ return d.isValid; }));
      // .data(nodeArray.filter(function(d){ return d.isValid; }), function(d) { return d ? d.nodeId : null; });
  ...
  var processNodeAddQ = function(callback) {
    ...
    if (nodeAddQReady && (nodeAddQ.length > 0)) {
      ...
      if (nodeIdHashMap.has(newNode.nodeId)){
        ...
        nodeSvgGroup.select("#" + currentNode.nodePoolId)
          .attr("nodeId", currentNode.nodeId);
          // .on("mouseover", nodeMouseOver)
          // .on("mouseout", nodeMouseOut)
          // .on("click", nodeClick);

        nodeLabelSvgGroup.select("#" + currentNode.nodePoolId + "_label")
          .attr("nodeId", currentNode.nodeId);
          // .on("mouseover", nodeMouseOver)
          // .on("mouseout", nodeMouseOut)
          // .on("click", nodeClick);
