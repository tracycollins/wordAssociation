  ...
  if (options) {
    ...
    console.log(
      MODULE_ID +
        ...
        threeceeUser.searchTermSet.size
    );
    ...
    console.log(
      chalkLog(
        MODULE_ID +
          ...
          threeceeUser.screenName +
          " | FOLLOW USER ID SET: " +
          threeceeUser.followUserIdSet.size
      )
    );
  } else {
    console.log(
            ...
            if (err.code == 88) {
              ...
              tcUtils.emitter.once(rateLimitEndEvent, function () {
                console.log(
                  chalkError(
                    MODULE_ID +
                      " | -X- TWITTER STREAM RATE LIMIT END" +
                      " | @" +
                      threeceeUser.screenName +
                      " | " +
                      getTimeStamp() +
...
async function initTwit() {
  console.log(
    chalkLog(MODULE_ID + " | INIT TWIT USER @" + threeceeUser.screenName)
  );
  ...
  console.log(
    chalkTwitter(
      MODULE_ID + " | INIT TWITTER USER" + " | NAME: " + threeceeUser.screenName
    )
  );
  ...
  try {
    ...
    console.log(
      chalkError(
        MODULE_ID +
          " | *** TWITTER GET FRIENDS IDS ERROR | NOT AUTHENTICATED" +
          " | @" +
          threeceeUser.screenName +
          " | " +
          getTimeStamp() +
...
async function initTwitterUser() {
  console.log(
    chalkTwitter(MODULE_ID + " | TWITTER USER: @" + threeceeUser.screenName)
  );
  ...
  return new Promise(function (resolve, reject) {
    ...
    try {
      if (threeceeUser.searchStream) {
        console.log(
          chalkAlert(MODULE_ID + " | !!! RESTARTING TWITTER SEARCH STREAM")
        );
      ...
      threeceeUser.searchStream = threeceeUser.twitStream.stream(
        "statuses/filter",
        filter
      );
        ...
        console.log(
          chalkTwitter(
            MODULE_ID +
              ...
              " | TWITTER RECONNECT" +
              " | RATE LIMIT: " +
              threeceeUser.stats.rateLimited +
              " | @" +
              threeceeUser.screenName
          )
        );
      ...
      threeceeUser.searchStream.on("tweet", function (tweetStatus) {
        ...
        if (
          filterRetweets &&
          (tweetStatus.retweeted_status || tweetStatus.is_quote_status)
        ) {
        ...
        if (
          tweetStatus.user.description &&
          tweetStatus.user.description !== undefined
        ) {
          const [userLangDescription] = lngDetector.detect(
            tweetStatus.user.description,
            1
          );
          ...
          if (
            ...
            parseFloat(userLangDescription[1]) >
              parseFloat(configuration.langDescriptionThreshold)
          ) {
            statsObj.filtered.languages += 1;
            return;
          }
          ...
          if (
            ignoredProfileWordsArray.some((word) =>
              tweetStatus.user.description.includes(word)
            )
          ) {
        ...
        if (
          tweetStatus.user.location &&
          tweetStatus.user.location !== undefined
        ) {
          const [userLangLocation] = lngDetector.detect(
            tweetStatus.user.location,
            1
          );
          ...
          if (
            ...
            parseFloat(userLangLocation[1]) >
              parseFloat(configuration.langLocationThreshold)
          ) {
            statsObj.filtered.languages += 1;
            return;
          }
...
async function initSearchTerms(params) {
  console.log(
    chalkTwitter(MODULE_ID + " | INIT TERMS | @" + threeceeUser.screenName)
  );
  ...
  try {
    ...
    console.log(
      chalkLog(
        MODULE_ID +
          " | ADDING " +
          threeceeUser.followUserScreenNameSet.size +
          " SCREEN NAMES TO TRACK SET"
      )
    );

    threeceeUser.searchTermSet = new Set([
      ...threeceeUser.searchTermSet,
      ...threeceeUser.followUserScreenNameSet,
    ]);

    if (threeceeUser.searchTermSet.size == 0) {
      console.log(
        chalkAlert(
          MODULE_ID + " | ??? NO SEACH TERMS |@" + threeceeUser.screenName
        )
      );
...
async function initAllowLocations() {
  console.log(
    chalkTwitter(
      MODULE_ID + " | INIT ALLOW LOCATIONS | @" + threeceeUser.screenName
    )
  );
  ...
  try {
    ...
    console.log(
      chalkError(MODULE_ID + " | *** INIT ALLOW LOCATIONS ERROR: " + err)
    );
    throw err;
  }
}
...
async function initIgnoreLocations() {
  console.log(
    chalkTwitter(
      MODULE_ID + " | INIT IGNORE LOCATIONS | @" + threeceeUser.screenName
    )
  );
  ...
  try {
    ...
    console.log(
      chalkError(MODULE_ID + " | *** INIT IGNORE LOCATIONS ERROR: " + err)
    );
    throw err;
  }
}
...
async function initIgnoreHashtags() {
  console.log(
    chalkTwitter(
      MODULE_ID + " | INIT IGNORE HASHTAGS | @" + threeceeUser.screenName
    )
  );
  ...
  try {
    ...
    console.log(
      chalkError(MODULE_ID + " | *** INIT IGNORE HASHTAGS ERROR: " + err)
    );
    throw err;
  }
}
...
async function initialize(cnf) {
  ...
  try {
    ...
    if (loadedConfigObj.TSS_VERBOSE_MODE !== undefined) {
      console.log(
        MODULE_ID +
          " | LOADED TSS_VERBOSE_MODE: " +
          loadedConfigObj.TSS_VERBOSE_MODE
      );
    ...
    if (loadedConfigObj.TSS_TEST_MODE !== undefined) {
      console.log(
        MODULE_ID + " | LOADED TSS_TEST_MODE: " + loadedConfigObj.TSS_TEST_MODE
      );
    ...
    if (
      ...
      console.log(
        MODULE_ID +
          " | LOADED DROPBOX_WORD_ASSO_DEFAULT_TWITTER_CONFIG_FOLDER: " +
          loadedConfigObj.DROPBOX_WORD_ASSO_DEFAULT_TWITTER_CONFIG_FOLDER
      );
      cnf.twitterConfigFolder =
        loadedConfigObj.DROPBOX_WORD_ASSO_DEFAULT_TWITTER_CONFIG_FOLDER;
    ...
    if (loadedConfigObj.DROPBOX_DEFAULT_SEARCH_TERMS_DIR !== undefined) {
      console.log(
        MODULE_ID +
          " | LOADED DROPBOX_DEFAULT_SEARCH_TERMS_DIR: " +
    ...
    if (loadedConfigObj.TSS_LANG_DES_THRESHOLD !== undefined) {
      console.log(
        MODULE_ID +
          " | LOADED TSS_LANG_DES_THRESHOLD: " +
          loadedConfigObj.TSS_LANG_DES_THRESHOLD
      );
    ...
    if (loadedConfigObj.TSS_LANG_LOC_THRESHOLD !== undefined) {
      console.log(
        MODULE_ID +
          " | LOADED TSS_LANG_LOC_THRESHOLD: " +
          loadedConfigObj.TSS_LANG_LOC_THRESHOLD
      );
    ...
    if (loadedConfigObj.DROPBOX_DEFAULT_SEARCH_TERMS_FILE !== undefined) {
      console.log(
        MODULE_ID +
          " | LOADED DROPBOX_DEFAULT_SEARCH_TERMS_FILE: " +
    ...
    if (loadedConfigObj.TSS_STATS_UPDATE_INTERVAL !== undefined) {
      console.log(
        MODULE_ID +
          " | LOADED TSS_STATS_UPDATE_INTERVAL: " +
...
function initTwitterSearch(cnf) {
  return new Promise(function (resolve) {
    ...
    console.log(
      chalkTwitter(
        MODULE_ID + " | " + getTimeStamp() + " | ENABLE TWEET STREAM"
      )
    );
...
async function initFollowQueue(params) {
  try {
    ...
    console.log(chalkTwitter("TSS" + " | FOLLOW QUEUE INTERVAL: " + interval));
...
process.on("message", async function (m) {
  ...
  switch (m.op) {
    ...
    case "USER_AUTHENTICATED":
      ...
      console.log(
        chalkLog(MODULE_ID + " | CURRENT AUTH\n" + jsonPrint(authObj))
      );

      threeceeUser.twitStream.setAuth({
      ...
      threeceeUser.twitterConfig.access_token_secret =
      ...
      console.log(
        chalkError(MODULE_ID + " | UPDATED AUTH\n" + jsonPrint(authObjNew))
      );
      ...
      await tcUtils.saveFile({
        ...
        obj: threeceeUser.twitterConfig,
      });
      ...
      try {
        ...
        console.log(
          chalkError(MODULE_ID + " | *** USER AUTHENTICATE ERROR: " + err)
        );
      }
      break;
...
setTimeout(async function () {
  ...
  try {
    try {
      ...
      if (err.status != 404) {
        console.log(
          chalkError(MODULE_ID + " | *** INIT ERROR\n" + jsonPrint(err))
        );
        quit();
      }
      console.log(
