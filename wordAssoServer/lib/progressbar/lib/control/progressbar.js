...
ProgressBar.getVersion = function () {
  return "1.2";
};
...
function ProgressBar($elementID, $options) {
  ...
  var _itemElements = [];
  var _width = "";
  var _height = ""; //Hızlı bulma

  var _exItemID = "";
  var _exItemIndex = ""; //Value kullanma
  ...
  var _initialModeEngine = function () {
    if (_items.length == 1) {
      ...
      var nextAlign = currentAlign == 0 ? 100 - _that.getOptionValue(_optionName.PERCENT) : 0;
  ...
  var _getItemIndexByID = function ($itemID) {
    ...
    var itemIndex = 0; //length 1 ise [0]
    ...
    if (_items.length != 1) {
      ...
      if (_items.length == 0) {
        ...
        //itemID boş ise [0]
        if (!$itemID) return 0; //hepsine bak
  ...
  var _calculateAvailablePercent = function ($percent, $itemIndex) {
    ...
    switch (item[_optionName.POSITION]) {
      case _optionValue.POSITION.RELATIVE:
        for (var i = 0, j = _items.length; i < j; i++) {
          if (i != $itemIndex && _items[i][_optionName.POSITION] == _optionValue.POSITION.RELATIVE) {
            totalUsedPercent += _items[i][_optionName.PERCENT];
          }
        }

        availablePercent = 100 - totalUsedPercent; //POSITION: RELATIVE olan nesneler arasında yer kalmaz ise COMPLETED yap.

        if ($percent >= availablePercent) {
          //var me = _that;
          setTimeout(function () {
            _element.dispatchEvent(new CustomEvent(_event.COMPLETED, {
              'detail': {
                'itemID': item.itemID,
                'me': _that
              }
  ...
  this.createItem = function ($item) {
    ...
    $item = $item || {}; //Yeni nesne için element oluştur

    var itemElement = _element.appendChild(document.createElement('div'));

    var itemID = $item[_optionName.ITEM_ID] = $item[_optionName.ITEM_ID] || ""; //Daha önce eklenmiş nesne var ise ID siz nesne ekleyemez.

    if (_items.length > 0 && itemID == "") return false; // DEFAULT değerleri set et.
    ...
    $item[_optionName.ALIGN] = $item[_optionName.ALIGN] || _defaultValue.ALIGN; //SPACE varsa POSITION: ABSOLUTE olmalı

    if ($item[_optionName.SPACE]) {
      $item[_optionName.POSITION] = _optionValue.POSITION.ABSOLUTE;
    } else {
      $item[_optionName.SPACE] = _defaultValue.SPACE; // (OK)ERROR: SPACE değeri olmayan bir ABSOLUTE item oluşturulamıyorudu.
    ...
    _itemElements.push(itemElement); //nesneyi oluşturulurken gizle
    ...
    this.setOptionValue(_optionName.ALIGN, $item[_optionName.ALIGN], itemID); // ALIGN değeri set edildiğinde SPACE değeri otomatik güncellenir.
    //if($item[_optionName.SPACE]) this.setOptionValue(_optionName.SPACE, $item[_optionName.SPACE], itemID);

    this.setOptionValue(_optionName.POSITION, $item[_optionName.POSITION], itemID); // PERCENT değeri set edildiğinde opacity değeri otomatik güncellenir.
  ...
  this.getPercentByValue = function ($value, $itemID, $maxValue) {
    ...
    if ($maxValue && $maxValue != this.getMaxValue()) this.setMaxValue($maxValue); //value değeri 0 - maxValue arasında olabilir.

    if ($value < 0) $value = 0;
    if ($value > this.getMaxValue()) $value = this.getMaxValue(); //itemID boş ise first olarak adlandır.
  ...
  this.setOptionValue = function ($optionName, $value, $itemID) {
    ...
    switch ($optionName) {
        ...
        setTimeout(function () {
          ...
          switch (item[_optionName.TYPE]) {
            ...
            case _optionValue.TYPE.LINE:
              ...
              //item[_optionName.PERCENT] = $value;
              me.setOptionValue(_optionName.SPACE, $value, $itemID); //Görünürlüğü ayarla
          ...
          setTimeout(function () {
            _element.dispatchEvent(new CustomEvent(_event.CHANGED, {
              'detail': {
                'itemID': $itemID,
                'me': me
              }
            ...
            switch ($value) {
              case 0:
                _element.dispatchEvent(new CustomEvent(_event.STARTING, {
                  'detail': {
                    'itemID': $itemID,
                    'me': me
                  }
      ...
      case _optionName.ALIGN:
        ...
        item[_optionName.ALIGN] = $value; //ALIGN değeri değişir ise SPACE değerini güncelle

        if (item[_optionName.POSITION] == _optionValue.POSITION.ABSOLUTE) //if(item[_optionName.SPACE])
      ...
      case _optionName.SPACE:
        ...
        if (item[_optionName.POSITION] != _optionValue.POSITION.ABSOLUTE) this.setOptionValue(_optionName.POSITION, _optionValue.POSITION.ABSOLUTE, $itemID); //Önceki değeri sıfırla

        itemElement.style.left = "";
        itemElement.style.right = "";
        itemElement.style[item[_optionName.ALIGN]] = $value + '%';
        item[_optionName.SPACE] = $value; // TODO: BAR ise PERCENT değerini tekrar güncelle, değişiklik olabilir.

        if (item[_optionName.TYPE] == _optionValue.TYPE.BAR) this.setOptionValue(_optionName.PERCENT, item[_optionName.PERCENT], $itemID);
        break;
        ...
        switch (item[_optionName.TYPE]) {
          case _optionValue.TYPE.BAR:
            ...
            if ($value == _optionValue.POSITION.RELATIVE) {
              itemElement.style.left = "";
              itemElement.style.right = ""; //SPACE değerini array içine yaz.
  ...
  this.initialMode = function (isInitialMode) {
    if (isInitialMode) {
      this.removeAll();
      this.setPercent(25); //this.setOptionValue(_optionName.OPACITY, 0.7);

      this.setOptionValue(_optionName.COLOR_ID, _optionValue.COLOR_ID.WHITE);
      var me = this; //Engine çalışmadan önce PERCENT set edilebilsin.
