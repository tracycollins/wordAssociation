...
const USER_PROFILE_PROPERTY_ARRAY = ["bannerImageUrl", "description", "location", "name", "profileUrl", "profileImageUrl", "screenName", "url"];
const DEFAULT_INPUT_TYPES = ["emoji", "friends", "hashtags", "images", "locations", "media", "mentions", "ngrams", "places", "sentiment", "urls", "userMentions", "words"];
...
if (userChangeCacheTtl === undefined) {
  userChangeCacheTtl = USER_CHANGE_CACHE_DEFAULT_TTL;
}
...
if (userChangeCacheCheckPeriod === undefined) {
  userChangeCacheCheckPeriod = USER_CHANGE_CACHE_CHECK_PERIOD;
}
...
function showStats(options) {
  ...
  if (options) {
    ...
    console.log(chalkLog(MODULE_ID_PREFIX + " | S" + " | ELPSD " + tcUtils.msToTime(statsObj.elapsed) + " | START " + moment(parseInt(statsObj.startTime)).format(compactDateTimeFormat) + " | UC$: " + userChangeCache.getStats().keys + " | UCATQ: " + processUserQueue.length + " | AUTO CHG M " + statsObj.autoChangeMatch + " MM: " + statsObj.autoChangeMismatch + " TOT: " + statsObj.autoChangeTotal + " RATE: " + statsObj.autoChangeMatchRate.toFixed(2)));
  }
}
...
function quit(message) {
  ...
  console.error(MODULE_ID_PREFIX + " | " + process.argv[1] + " | " + moment().format(compactDateTimeFormat) + " | **** QUITTING" + " | CAUSE: " + msg + " | PID: " + process.pid);
...
function saveCacheExpired(file, fileObj) {
  debug(chalkLog("XXX $ SAVE" + " [" + saveCache.getStats().keys + "]" + " | " + file));
...
saveCache.on("set", function (file, fileObj) {
  debug(chalkLog(MODULE_ID_PREFIX + " | $$$ SAVE CACHE" + " [" + saveCache.getStats().keys + "]" + " | " + fileObj.folder + "/" + file));
});
  ...
  saveFileQueueInterval = setInterval(async function () {
    if (!statsObj.queues.saveFileQueue.busy && saveFileQueue.length > 0) {
      ...
      try {
        ...
        console.log(chalkLog(MODULE_ID_PREFIX + " | SAVED FILE" + " [Q: " + saveFileQueue.length + "] " + " [$: " + saveCache.getStats().keys + "] " + saveFileObj.folder + "/" + saveFileObj.file));
        statsObj.queues.saveFileQueue.busy = false;
      } catch (err) {
        console.log(chalkError(MODULE_ID_PREFIX + " | *** SAVE FILE ERROR ... RETRY" + " | ERROR: " + err + " | " + saveFileObj.folder + "/" + saveFileObj.file));
...
function initStatsUpdate() {
  return new Promise(function (resolve, reject) {
    try {
      ...
      saveFileQueue.push({
        folder: configuration.statsFolder,
        file: configuration.statsFile,
        obj: statsObj
      });
...
function printUser(params) {
  ...
  if (params.verbose) {
    ...
    text = user.nodeId + " | @" + user.screenName + " | " + user.name + " | LG " + user.lang + " | FW " + user.followersCount + " | FD " + user.friendsCount + " | T " + user.statusesCount + " | M  " + user.mentions + " | LS " + tcUtils.getTimeStamp(user.lastSeen) + " | FWG " + user.following + " | LC " + user.location + " | CN " + user.categorizeNetwork + " | C M " + formatCategory(user.category) + " A " + formatCategory(user.categoryAuto);
    return text;
  }
}
...
function printNetworkObj(title, nn, format) {
  ...
  console.log(chalkFormat(title + " | RK: " + rank + " | PRK: " + previousRank + " | SR: " + successRate.toFixed(2) + "%" + " | MR: " + matchRate.toFixed(2) + "%" + " | OAMR: " + overallMatchRate.toFixed(2) + "%" + " | RMR: " + runtimeMatchRate.toFixed(2) + "%" // + " | CR: " + tcUtils.getTimeStamp(nn.createdAt)
  + " | TC:  " + nn.testCycles + " | TH: " + testCycleHistory.length + " |  " + nn.inputsId + " | " + nn.networkId));
  return;
}
...
async function processTweetObj(params) {
  ...
  for (const entityType of DEFAULT_INPUT_TYPES) {
    ...
    if (empty(tweetObj[entityType])) {
      continue;
    }

    if (tweetObj[entityType].length == 0) {
      continue;
    }
  ...
  for (const tweet of tweets) {
    ...
    if (forceFetch || !user.tweets.tweetIds.includes(tweet.id_str.toString())) {
      try {
        ...
        user.tweetHistograms = await processTweetObj({
          tweetObj: tweetObj,
        ...
        if (configuration.testMode || configuration.verbose) {
          console.log(chalkTwitter(MODULE_ID_PREFIX + " | +++ PROCESSED TWEET" + " | FORCE: " + forceFetch + " [ P/H/T " + statsObj.twitter.tweetsProcessed + "/" + statsObj.twitter.tweetsHits + "/" + statsObj.twitter.tweetsTotal + "]" + " | TW: " + tweet.id_str + " | SINCE: " + user.tweets.sinceId + " | TWs: " + user.tweets.tweetIds.length + " | @" + user.screenName));
        }
      } catch (err) {
      ...
      if (configuration.testMode || configuration.verbose) {
        console.log(chalkInfo(MODULE_ID_PREFIX + " | ... TWEET ALREADY PROCESSED" + " [ P/H/T " + statsObj.twitter.tweetsProcessed + "/" + statsObj.twitter.tweetsHits + "/" + statsObj.twitter.tweetsTotal + "]" + " | TW: " + tweet.id_str + " | TWs: " + user.tweets.tweetIds.length + " | @" + user.screenName));
      }
    }
  }

  if (forceFetch || configuration.testMode || configuration.verbose) {
    console.log(chalkLog(MODULE_ID_PREFIX + " | +++ Ts" + " | FORCE: " + forceFetch + " [ P/H/T " + statsObj.twitter.tweetsProcessed + "/" + statsObj.twitter.tweetsHits + "/" + statsObj.twitter.tweetsTotal + "]" + " | Ts: " + user.tweets.tweetIds.length + " | @" + user.screenName));
  }
...
async function processUserTweets(params) {
  ...
  try {
    ...
    if (tweetHistogramsEmpty) {
      console.log(chalkLog(MODULE_ID_PREFIX + " | USER Ts" + " | " + printUser({
        user: processedUser
      })));
      debug(chalkLog(MODULE_ID_PREFIX + " | USER Ts" + " | SINCE: " + processedUser.tweets.sinceId + " | TWEETS: " + processedUser.tweets.tweetIds.length));
...
async function updateUserTweets(params) {
  try {
    ...
    if (configuration.testFetchTweetsMode || !userTweetFetchSet.has(user.nodeId) && (histogramIncompleteFlag || user.priorityFlag)) {
      userTweetFetchSet.add(user.nodeId);

      if (configuration.testFetchTweetsMode) {
        console.log(chalkAlert(MODULE_ID_PREFIX + " | updateUserTweets | !!! TEST MODE FETCH TWEETS" + " | @" + user.screenName));
      } else {
        debug(chalkInfo(MODULE_ID_PREFIX + " | PRI FETCH TWEETS" + " | @" + user.screenName));
      }

      if (histogramIncompleteFlag) {
        user.tweetHistograms = {};
      }
    ...
    if (user.tweets.tweetIds.length > DEFAULT_MAX_USER_TWEETIDS) {
      ...
      debug(chalkLog(MODULE_ID_PREFIX + " | ---  TWEETS > MAX TWEETIDS" + " | " + user.nodeId + " | @" + user.screenName + " | " + length + " TWEETS" + " | REMOVE: " + removeNumber));
    ...
    if (twitterDeleteUserErrorCodesArray.includes(err.code)) {
      console.log(chalkError(MODULE_ID_PREFIX + " | XXX DELETE USER" + " | ERROR STATUS CODE: " + err.statusCode + " | NID: " + params.user.nodeId + " | @" + params.user.screenName));
      await global.wordAssoDb.User.deleteOne({
...
async function updateUserFriends(params) {
  ...
  try {
    const friendsIdsObj = await tcUtils.fetchUserFriends({
      user: user
    });
  ...
  return new Promise(function (resolve) {
    ...
    processUserQueueInterval = setInterval(async function () {
      if (!processUserQueueBusy && processUserQueue.length > 0) {
        ...
        try {
          userDoc = await global.wordAssoDb.User.findOne({
            nodeId: queueObj.user.nodeId
          });

          if (!userDoc) {
            processUserQueueBusy = false;
          } else {
            user = await tcUtils.encodeHistogramUrls({
              user: userDoc
            });
            ...
            processedUser = await processUser({
              user: user
            });
            statsObj.user.processed += 1;

            if (queueObj.priorityFlag) {
              console.log(chalkInfo(MODULE_ID_PREFIX + " | PRI PRCSSD" + " [ PRCSSD: " + statsObj.user.processed + " / PUQ: " + processUserQueue.length + "]" + " | PRI: " + queueObj.priorityFlag + " | NID: " + processedUser.nodeId + " | @" + processedUser.screenName + " | N: " + processedUser.name + " | CN: " + processedUser.categorizeNetwork + " | CV: " + processedUser.categoryVerified + " | CM: " + formatCategory(processedUser.category) + " A: " + formatCategory(processedUser.categoryAuto)));
            }
          ...
          if (err.code) {
            await tcUtils.handleTwitterError({
              err: err,
              user: queueObj.user
            });
            process.send({
            ...
            console.log(chalkError("*** ERROR initProcessUserQueueInterval" + " | NID: " + queueObj.user.nodeId + " | @" + queueObj.user.screenName + " | " + err));
          }
...
async function initialize(cnf) {
  ...
  try {
    ...
    const twitterConfig = await tcUtils.loadFileRetry({
      folder: configuration.twitterConfigFolder,
      file: configuration.twitterConfigFile
    });
    cnf.twitterConfig = {};
    cnf.twitterConfig = twitterConfig;
    console.log(MODULE_ID_PREFIX + " | " + chalkInfo(tcUtils.getTimeStamp() + " | TWITTER CONFIG FILE " + configuration.twitterConfigFolder + configuration.twitterConfigFile + "\n" + tcUtils.jsonPrint(cnf.twitterConfig)));
    return cnf;
  } catch (err) {
...
async function updateNetworkRuntimeStats() {
  ...
  for (const nnId of Object.keys(networkStatsObj.networks)) {
    ...
    printNetworkObj(MODULE_ID_PREFIX + " | +++ UPDATE NN RT", networkDoc.toObject(), chalk.black);
  }
...
async function generateAutoCategory(p) {
  try {
    ...
    if (statsObj.currentBestRuntimeNetwork.rank < currentBestRuntimeNetwork.rank) {
      printNetworkObj("RNT | +++ UPDATE BEST NN" + " | @" + user.screenName + " | CM: " + formatCategory(user.category), currentBestRuntimeNetwork, chalk.black);
      ...
      printNetworkObj(MODULE_ID_PREFIX + " | NN" + " | @" + user.screenName + " | CM: " + formatCategory(user.category), currentBestRuntimeNetwork, chalk.black);
      await nnTools.printNetworkResults();
      await updateNetworkRuntimeStats();
    }

    if (configuration.verbose || statsObj.currentBestRuntimeNetwork.rank < currentBestRuntimeNetwork.rank) {
      console.log(MODULE_ID_PREFIX + " | BEST RUNTIME NN" + " | RANK: " + currentBestRuntimeNetwork.rank + " | MR: " + currentBestRuntimeNetwork.matchRate.toFixed(2) + "%" + " | " + currentBestRuntimeNetwork.meta.match + "/" + currentBestRuntimeNetwork.meta.total + " | MATCH: " + currentBestRuntimeNetwork.meta.matchFlag + " | " + currentBestRuntimeNetwork.networkId + " | IN: " + currentBestRuntimeNetwork.inputsId + " | OUT: " + currentBestRuntimeNetwork.meta.output);
    }
    ...
    if (configuration.verbose || user.categoryAuto != currentBestRuntimeNetwork.meta.categoryAuto) {
      console.log(chalkLog(text + " | " + currentBestRuntimeNetwork.networkId + " | AUTO CHG M/MM/TOT: " + statsObj.autoChangeMatch + "/" + statsObj.autoChangeMismatch + "/" + statsObj.autoChangeTotal + " | " + statsObj.autoChangeMatchRate.toFixed(2) + "%" + " | V: " + user.categoryVerified + " | M: " + formatCategory(user.category) + " | A: " + formatCategory(user.categoryAuto) + " --> " + formatCategory(currentBestRuntimeNetwork.meta.categoryAuto) + " | @" + user.screenName));
    }
...
const processUserPickArray = ["ageDays", "bannerImageUrl", "category", "categoryAuto", "categorizeNetwork", "categoryVerified", "createdAt", "description", "followersCount", "following", "friendsCount", "ignored", "isBot", "isTopTerm", "isTweeter", "isTweetSource", "lastSeen", "location", "mentions", "name", "nodeId", "nodeType", "profileImageUrl", "rate", "rateMax", "rateMaxTime", "screenName", "statusesCount", "tweetsPerDay", "userId", "verified"];
...
async function processUser(params) {
  ...
  try {
    const user = params.user; // user.following = true;
    ...
    if (configuration.enableFetchUserFriends && (!user.friends || user.friends.length === undefined || user.friends.length === 0)) {
      updatedFriendsUser = await updateUserFriends({
        user: user
      });
    }

    const updatedTweetsUser = await updateUserTweets({
      user: updatedFriendsUser
    });
    ...
    const u = pick(prevPropsUser.toObject(), processUserPickArray); // KLUDGE!! why these aren't passed thru in toObjec()?

    u.ageDays = prevPropsUser.ageDays;
    u.tweetsPerDay = prevPropsUser.tweetsPerDay;
    u.tweetHistograms = {};
    u.profileHistograms = {};
    u.friends = []; // const u = prevPropsUser.toJSON();
    ...
    for (const nnId of configuration.bestNetworkIdArray) {
      ...
      if (nn) {
        if (nn.testCycleHistory && nn.testCycleHistory !== undefined && nn.testCycleHistory.length > 0) {
          ...
          console.log(chalkLog(MODULE_ID_PREFIX + " | PREV RANK " + nn.previousRank + " | " + nn.networkId));
        }
...
async function fixIncorrectNetworkMetaData(params) {
  try {
    ...
    if (params.networkObj.evolve && params.networkObj.evolve.options.networkTechnology && params.networkObj.evolve.options.networkTechnology !== params.networkObj.networkTechnology) {
      console.log(chalkAlert(MODULE_ID_PREFIX + " | !!! INCORRECT NETWORK TECH | CHANGE " + params.networkObj.networkTechnology + " -> " + params.networkObj.evolve.options.networkTechnology + " | " + params.networkObj.networkId));
    ...
    if (params.networkObj.evolve && params.networkObj.evolve.options.binaryMode !== undefined && params.networkObj.evolve.options.binaryMode !== params.networkObj.binaryMode) {
      console.log(chalkAlert(MODULE_ID_PREFIX + " | !!! INCORRECT BINARY MODE | CHANGE " + params.networkObj.binaryMode + " -> " + params.networkObj.evolve.options.binaryMode + " | " + params.networkObj.networkId));
    ...
    if (incorrectUpdateFlag) {
      console.log(chalkLog(MODULE_ID_PREFIX + " | ... SAVING UPDATED INCORRECT NN META DATA" + " | INCORRECT FLAG: " + incorrectUpdateFlag + " | " + params.networkObj.networkId));
  ...
  switch (m.op) {
    case "INIT":
      ...
      if (empty(primaryNnObj)) {
        console.log(chalkError(MODULE_ID_PREFIX + " | *** UNDEFINED NETWORK " + tcUtils.jsonPrint(m)));
        quit();
        break;
      }
      ...
      if (primaryNnObj.testCycleHistory && primaryNnObj.testCycleHistory !== undefined && primaryNnObj.testCycleHistory.length > 0) {
        ...
        console.log(chalkLog(MODULE_ID_PREFIX + " | PRIMARY NN PREV RANK " + primaryNnObj.previousRank + " | " + primaryNnObj.networkId));
      }

      const pNnObj = await fixIncorrectNetworkMetaData({
        networkObj: primaryNnObj,
        updateDatabaseOnly: true
      });
      primaryNetworkObj = await nnTools.convertNetwork({
        networkObj: pNnObj
      });
      await nnTools.deleteAllNetworks();
      await nnTools.loadNetwork({
        networkObj: primaryNetworkObj
      });
      nnTools.setPrimaryNeuralNetwork(primaryNetworkObj.networkId); // await nnTools.setMaxInputHashMap(m.maxInputHashMap);
      ...
      await tcUtils.initTwitter({
        twitterConfig: m.twitterConfig
      });
      await initProcessUserQueueInterval(configuration.processUserQueueInterval);
      console.log(chalkBlueBold("===============================\n" + MODULE_ID_PREFIX + " | INIT" + " | TITLE: " + process.title + " | PRIMARY NETWORK: " + primaryNetworkObj.networkId + " | USER PROFILE ONLY FLAG: " + configuration.userProfileOnlyFlag + " | ENABLE GEOCODE: " + configuration.enableGeoCode + " | FORCE GEOCODE: " + configuration.forceGeoCode + " | ENABLE IMAGE ANALYSIS: " + configuration.enableImageAnalysis + " | FORCE IMAGE ANALYSIS: " + configuration.forceImageAnalysis + " | ENABLE LANG ANALYSIS: " + configuration.enableLanguageAnalysis + " | FORCE LANG ANALYSIS: " + configuration.forceLanguageAnalysis // + "\nWAS | TFC | INIT | MAX INPUT HM KEYS: " + Object.keys(nnTools.getMaxInputHashMap())
    ...
    case "NETWORK":
      ...
      statsObj.autoChangeMatch = 0;
      statsObj.autoChangeMismatch = 0;
      console.log(chalkBlueBold(MODULE_ID_PREFIX + " | >>> SET PRIMARY NETWORK" + " | NETWORK: " + primaryNetworkObj.networkId + " | INPUTS ID: " + primaryNetworkObj.inputsId));
      break;

    case "FORCE_IMAGE_ANALYSIS":
      configuration.forceImageAnalysis = m.forceImageAnalysis;
      console.log(chalkInfo(MODULE_ID_PREFIX + " | +++ FORCE_IMAGE_ANALYSIS" + " | FORCE IMAGE ANALYSIS: " + configuration.forceImageAnalysis));
      break;
    ...
    case "NORMALIZATION":
      await nnTools.setNormalization(m.normalization);
      console.log(chalkInfo(MODULE_ID_PREFIX + " | +++ NORMALIZATION" + " | NORMALIZATION: " + Object.keys(nnTools.getNormalization())));
      break;

    case "USER_AUTHENTICATED":
      console.log(chalkInfo(MODULE_ID_PREFIX + " | USER_AUTHENTICATED" + " | @" + m.user.screenName + " | UID: " + m.user.userId + " | TOKEN: " + m.token + " | TOKEN SECRET: " + m.tokenSecret));
      ...
      if (twitterUserObj !== undefined) {
        ...
        twitterUserObj.twit.setAuth({
          access_token: m.token,
          access_token_secret: m.tokenSecret
        });
    ...
    case "USER_CATEGORIZE":
      if (m.priorityFlag) {
        console.log(chalkInfo(MODULE_ID_PREFIX + " | PRI CAT" + " [PUQ: " + processUserQueue.length + "]" + " | NID: " + m.user.nodeId + " | @" + m.user.screenName + " | CN: " + m.user.categorizeNetwork + " | CV: " + m.user.categoryVerified + " | CM: " + m.user.category + " | CA: " + m.user.categoryAuto));
      }
      ...
      if (m.priorityFlag && configuration.verbose) {
        if (cacheObj) {
          console.log(chalk.green(MODULE_ID_PREFIX + " | CAT $ HIT" + " [UC$: " + userChangeCache.getStats().keys + "]" + " [PUQ: " + processUserQueue.length + "]" + " | SEARCH: " + m.searchMode + " | NID: " + m.user.nodeId + " | @" + m.user.screenName + " | CN: " + m.user.categorizeNetwork + " | CV: " + m.user.categoryVerified + " | CM: " + m.user.category + " | CA: " + m.user.categoryAuto));
        } else {
          console.log(chalk.gray(MODULE_ID_PREFIX + " | CAT $ MISS" + " [UC$: " + userChangeCache.getStats().keys + "]" + " [PUQ: " + processUserQueue.length + "]" + " | SEARCH: " + m.searchMode + " | NID: " + m.user.nodeId + " | @" + m.user.screenName + " | CN: " + m.user.categorizeNetwork + " | CV: " + m.user.categoryVerified + " | CM: " + m.user.category + " | CA: " + m.user.categoryAuto));
        }
      }
      ...
      if (m.priorityFlag || cacheObj === undefined && processUserQueue.length < USER_PROCESS_QUEUE_MAX_LENGTH) {
        try {
          ...
          if (m.priorityFlag) {
            ...
            userChangeCache.set(user.nodeId, {
              user: user,
              timeStamp: moment().valueOf()
            });
            processUserQueue.push({
              user: user,
              priorityFlag: m.priorityFlag,
              searchMode: m.searchMode
            });
          ...
          if (m.priorityFlag) {
            ...
            userChangeCache.set(m.user.nodeId, {
              user: m.user,
              timeStamp: moment().valueOf()
            });
            processUserQueue.push({
    ...
    default:
      console.error(chalkLog(MODULE_ID_PREFIX + " | TWP | *** UNKNOWN OP" + " | INTERVAL: " + m.op));
  }
});
