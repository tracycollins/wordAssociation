...
console.log(
  MODULE_ID_PREFIX +
    " | userChange CACHE CHECK PERIOD: " +
    userChangeCacheCheckPeriod +
    " SECONDS"
);
...
async function connectDb() {
  try {
    ...
    tweetServerController = new TweetServerController(
...
configuration.configDefaultFolder = path.join(
  DROPBOX_ROOT_FOLDER,
...
configuration.twitterConfigFolder = path.join(
  DROPBOX_ROOT_FOLDER,
  "config/twitter"
);
    ...
    console.log(
      chalkLog(
        MODULE_ID_PREFIX +
          ...
          " | UC$: " +
          userChangeCache.getStats().keys +
          " | UCATQ: " +
          processUserQueue.length +
          " | AUTO CHG M " +
          statsObj.autoChangeMatch +
          " MM: " +
          statsObj.autoChangeMismatch +
          " TOT: " +
          statsObj.autoChangeTotal +
          " RATE: " +
  ...
  console.log(
    chalkLog(
      MODULE_ID_PREFIX +
        ...
        tcUtils.msToTime(cnf.saveFileQueueInterval)
    )
  );
...
function printUser(params) {
  ...
  if (params.verbose) {
    ...
    text =
      ...
      user.mentions +
      " | LS " +
      tcUtils.getTimeStamp(user.lastSeen) +
      " | FWG " +
      user.following +
      " | LC " +
      user.location +
...
async function processTweetObj(params) {
  ...
  for (const entityType of DEFAULT_INPUT_TYPES) {
    if (!entityType || entityType === undefined) {
      console.log(
        chalkAlert(
          MODULE_ID_PREFIX + " | ??? UNDEFINED TWEET entityType: ",
          entityType
        )
      );
      continue;
    ...
    for (const entityObj of tweetObj[entityType]) {
      if (empty(entityObj)) {
        debug(
          chalkAlert(
            MODULE_ID_PREFIX +
              " | *** processTweetObj EMPTY ENTITY | ENTITY TYPE: " +
              entityType +
              " | entityObj: " +
              tcUtils.jsonPrint(entityObj)
          )
        );
        continue;
      }
      ...
      if (empty(entityObj.nodeId)) {
        debug(
          chalkAlert(
            MODULE_ID_PREFIX +
              " | *** processTweetObj UNDEFINED NODE ID | ENTITY TYPE: " +
              entityType +
              " | entityObj: " +
              tcUtils.jsonPrint(entityObj)
          )
      ...
      switch (entityType) {
        ...
        case "userMentions":
          if (empty(entityObj.screenName)) {
            console.log(
              chalkAlert(
                MODULE_ID_PREFIX +
                  " | *** processTweetObj UNDEFINED SCREEN NAME | ENTITY TYPE: " +
                  entityType +
                  " | entityObj: " +
                  tcUtils.jsonPrint(entityObj)
              )
        ...
        default:
          console.log(
            chalkError(
              MODULE_ID_PREFIX +
                " | *** processTweetObj UNKNOWN ENTITY TYPE: " +
                entityType
            )
          );
          continue;
  ...
  for (const tweet of tweets) {
    ...
    if (forceFetch || !user.tweets.tweetIds.includes(tweet.id_str.toString())) {
      try {
        ...
        console.log(
          chalkError(MODULE_ID_PREFIX + " | updateUserTweets ERROR: " + err)
        );
        throw err;
      }
    } else {
  ...
  try {
    ...
    if (tweetHistogramsEmpty) {
      ...
      debug(
        chalkLog(
          MODULE_ID_PREFIX +
            " | >>> processUserTweetArray USER TWEET HISTOGRAMS" +
            "\n" +
      ...
      debug(
        chalkLog(
          MODULE_ID_PREFIX +
            " | >>> processUserTweetArray USER PROFILE HISTOGRAMS" +
            "\n" +
    ...
    console.log(
      chalkError(
        MODULE_ID_PREFIX + " | *** processUserTweetArray ERROR: " + err
      )
    );
    throw err;
  }
...
async function updateUserTweets(params) {
  try {
    ...
    if (
      ...
      if (configuration.testFetchTweetsMode) {
        console.log(
          chalkAlert(
            MODULE_ID_PREFIX +
              " | updateUserTweets | !!! TEST MODE FETCH TWEETS" +
              " | @" +
              user.screenName
          )
        );
        ...
        debug(
          chalkInfo(
            MODULE_ID_PREFIX + " | PRI FETCH TWEETS" + " | @" + user.screenName
          )
        );
      }
    ...
    console.log(
      chalkError(
        MODULE_ID_PREFIX + " | *** updateUserTweets ERROR CODE: " + err.code
      )
    );
    console.log(
      chalkError(
        MODULE_ID_PREFIX +
          " | *** updateUserTweets ERROR STATUS CODE: " +
          err.statusCode
      )
    );
...
function initProcessUserQueueInterval(interval) {
  return new Promise(function (resolve) {
    console.log(
      chalkBlue(
        MODULE_ID_PREFIX +
          " | INIT PROCESS USER QUEUE INTERVAL | " +
          interval +
          " MS"
      )
    );
      ...
      if (!processUserQueueBusy && processUserQueue.length > 0) {
        ...
        try {
          ...
          if (!userDoc) {
            ...
            user.latestTweets = _.union(
              userDoc.latestTweets,
              user.latestTweets
            );
              ...
              console.log(
                chalkInfo(
                  MODULE_ID_PREFIX +
                    ...
                    "]" +
                    " | PRI: " +
                    queueObj.priorityFlag +
                    " | NID: " +
                    processedUser.nodeId +
                    " | @" +
                    processedUser.screenName +
                    " | N: " +
                    processedUser.name +
                    " | CN: " +
                    processedUser.categorizeNetwork +
                    " | CV: " +
                    processedUser.categoryVerified +
                    " | CM: " +
                    formatCategory(processedUser.category) +
                    " A: " +
                    formatCategory(processedUser.categoryAuto)
                )
              );
            }
          ...
          if (err.code) {
            ...
            console.log(
              chalkError(
                "*** ERROR initProcessUserQueueInterval" +
                  " | NID: " +
                  queueObj.user.nodeId +
                  " | @" +
...
async function initWatchConfig() {
  ...
  watch.createMonitor(
    configuration.configDefaultFolder,
    watchOptions,
    function (monitor) {
  ...
  try {
    ...
    console.log(
      MODULE_ID_PREFIX +
        ...
        configuration.configDefaultFolder +
        "/" +
        configuration.configDefaultFile +
        "\n" +
    ...
    if (loadedConfigObj.TFE_VERBOSE_MODE !== undefined) {
      console.log(
        MODULE_ID_PREFIX +
          " | LOADED TFE_VERBOSE_MODE: " +
          loadedConfigObj.TFE_VERBOSE_MODE
      );
    ...
    if (loadedConfigObj.TFE_GLOBAL_TEST_MODE !== undefined) {
      console.log(
        MODULE_ID_PREFIX +
          " | LOADED TFE_GLOBAL_TEST_MODE: " +
          loadedConfigObj.TFE_GLOBAL_TEST_MODE
      );
    ...
    if (loadedConfigObj.TFE_TEST_MODE !== undefined) {
      console.log(
        MODULE_ID_PREFIX +
          " | LOADED TFE_TEST_MODE: " +
          loadedConfigObj.TFE_TEST_MODE
      );
    ...
    if (loadedConfigObj.TFE_ENABLE_FETCH_USER_FRIENDS !== undefined) {
      console.log(
        MODULE_ID_PREFIX +
          " | LOADED TFE_ENABLE_FETCH_USER_FRIENDS: " +
          loadedConfigObj.TFE_ENABLE_FETCH_USER_FRIENDS
      );
      cnf.enableFetchUserFriends =
        loadedConfigObj.TFE_ENABLE_FETCH_USER_FRIENDS;
    }
    ...
    if (loadedConfigObj.TFE_ENABLE_GEOCODE !== undefined) {
      console.log(
        MODULE_ID_PREFIX +
          " | LOADED TFE_ENABLE_GEOCODE: " +
          loadedConfigObj.TFE_ENABLE_GEOCODE
      );
    ...
    if (loadedConfigObj.TFE_STATS_UPDATE_INTERVAL !== undefined) {
      console.log(
        MODULE_ID_PREFIX +
          " | LOADED TFE_STATS_UPDATE_INTERVAL: " +
          loadedConfigObj.TFE_STATS_UPDATE_INTERVAL
      );
    ...
    configArgs.forEach(function (arg) {
      console.log(
        MODULE_ID_PREFIX + " | FINAL CONFIG | " + arg + ": " + cnf[arg]
      );
    });
...
async function generateAutoCategory(p) {
  try {
    ...
    if (
      ...
      (currentBestRuntimeNetwork.meta.total > 0 &&
        currentBestRuntimeNetwork.meta.total % 1000 === 0)
    ) {
      printNetworkObj(
        MODULE_ID_PREFIX +
          " | NN" +
    ...
    if (
      ...
      console.log(
        MODULE_ID_PREFIX +
          " | BEST RUNTIME NN" +
          " | RANK: " +
          currentBestRuntimeNetwork.rank +
          " | MR: " +
          currentBestRuntimeNetwork.matchRate.toFixed(2) +
          "%" +
          " | " +
          currentBestRuntimeNetwork.meta.match +
          "/" +
          currentBestRuntimeNetwork.meta.total +
          " | MATCH: " +
          currentBestRuntimeNetwork.meta.matchFlag +
          " | " +
          currentBestRuntimeNetwork.networkId +
          " | IN: " +
          currentBestRuntimeNetwork.inputsId +
          " | OUT: " +
          currentBestRuntimeNetwork.meta.output
      );
    }
    ...
    if (
      user.category &&
      user.category !== "none" &&
      currentBestRuntimeNetwork.meta.categoryAuto == user.category
    ) {
      statsObj.autoChangeTotal += 1;
      statsObj.autoChangeMatch += 1;
      statsObj.autoChangeMatchRate =
        100 * (statsObj.autoChangeMatch / statsObj.autoChangeTotal);
      ...
      console.log(
        chalkLog(
          text +
            ...
            formatCategory(user.categoryAuto) +
            " --> " +
            formatCategory(currentBestRuntimeNetwork.meta.categoryAuto) +
            " | @" +
            user.screenName
        )
      );
    ...
    console.log(
      chalkError(MODULE_ID_PREFIX + " | *** generateAutoCategory ERROR: " + err)
    );
    throw err;
  }
}
...
async function updatePreviousUserProps(params) {
  try {
    ...
    console.log(
      chalkError(
        MODULE_ID_PREFIX + " | *** updatePreviousUserProps ERROR: " + err
      )
    );
    throw err;
  }
...
async function processUser(params) {
  if (userServerController === undefined) {
    console.log(
      chalkError(
        MODULE_ID_PREFIX + " | *** processUser userServerController UNDEFINED"
      )
    );
  ...
  try {
    ...
    if (
      configuration.enableFetchUserFriends &&
      (!user.friends ||
        user.friends.length === undefined ||
    ...
    console.log(
      chalkError(MODULE_ID_PREFIX + " | *** processUser ERROR: " + err)
    );
    throw err;
  }
}
...
async function loadNetworks() {
  console.log(
    chalkLog(
      MODULE_ID_PREFIX +
        " | ... LOADING BEST NETWORKS FROM " +
        configuration.configDefaultFolder +
        "/" +
        configuration.bestNetworkIdArrayFile
    )
  );
  ...
  if (
    configuration.bestNetworkIdArray &&
    configuration.bestNetworkIdArray.length > 0
  ) {
    console.log(
      chalkLog(
        MODULE_ID_PREFIX +
          " | ... LOADING BEST NETWORKS: " +
          configuration.bestNetworkIdArray.length
      )
    );
    ...
    for (const nnId of configuration.bestNetworkIdArray) {
      ...
      if (nn) {
        if (
          nn.testCycleHistory &&
          nn.testCycleHistory !== undefined &&
          nn.testCycleHistory.length > 0
        ) {
          nn.previousRank =
            nn.testCycleHistory[nn.testCycleHistory.length - 1].rank;
          console.log(
            chalkLog(
              MODULE_ID_PREFIX +
                " | PREV RANK " +
...
process.on("message", async function (m) {
  ...
  switch (m.op) {
    case "INIT":
      ...
      configuration.bestNetworkIdArrayFile =
        m.bestNetworkIdArrayFile !== undefined
          ? m.bestNetworkIdArrayFile
          : configuration.bestNetworkIdArrayFile;
      configuration.userProfileOnlyFlag =
        m.userProfileOnlyFlag !== undefined
          ? m.userProfileOnlyFlag
          : configuration.userProfileOnlyFlag;
      configuration.enableGeoCode =
        m.enableGeoCode !== undefined
          ? m.enableGeoCode
          : configuration.enableGeoCode;
      configuration.forceGeoCode =
        m.forceGeoCode !== undefined
          ? m.forceGeoCode
          : configuration.forceGeoCode;
      configuration.enableImageAnalysis =
        m.enableImageAnalysis !== undefined
          ? m.enableImageAnalysis
          : configuration.enableImageAnalysis;
      configuration.forceImageAnalysis =
        m.forceImageAnalysis !== undefined
          ? m.forceImageAnalysis
          : configuration.forceImageAnalysis;
      configuration.enableLanguageAnalysis =
        m.enableLanguageAnalysis !== undefined
          ? m.enableLanguageAnalysis
          : configuration.enableLanguageAnalysis;
      configuration.forceLanguageAnalysis =
        m.forceLanguageAnalysis !== undefined
          ? m.forceLanguageAnalysis
          : configuration.forceLanguageAnalysis;
      statsObj.autoChangeTotal = 0;
      statsObj.autoChangeMatchRate = 0;
      statsObj.autoChangeMatch = 0;
      ...
      if (
        primaryNnObj.testCycleHistory &&
        primaryNnObj.testCycleHistory !== undefined &&
        primaryNnObj.testCycleHistory.length > 0
      ) {
        primaryNnObj.previousRank =
          primaryNnObj.testCycleHistory[
            primaryNnObj.testCycleHistory.length - 1
          ].rank;
        console.log(
          chalkLog(
            MODULE_ID_PREFIX +
              " | PRIMARY NN PREV RANK " +
              primaryNnObj.previousRank +
              " | " +
      ...
      await loadNetworks();
      await tcUtils.setEnableLanguageAnalysis(
        configuration.enableLanguageAnalysis
      );
      ...
      await initProcessUserQueueInterval(
        configuration.processUserQueueInterval
      );
      console.log(
        chalkBlueBold(
          "===============================\n" +
            ...
            process.title +
            " | PRIMARY NETWORK: " +
            primaryNetworkObj.networkId +
            " | USER PROFILE ONLY FLAG: " +
            configuration.userProfileOnlyFlag +
            " | ENABLE GEOCODE: " +
            configuration.enableGeoCode +
            " | FORCE GEOCODE: " +
            ...
            configuration.forceImageAnalysis +
            " | ENABLE LANG ANALYSIS: " +
            configuration.enableLanguageAnalysis +
            " | FORCE LANG ANALYSIS: " +
            configuration.forceLanguageAnalysis + // + "\nWAS | TFC | INIT | MAX INPUT HM KEYS: " + Object.keys(nnTools.getMaxInputHashMap())
            "\nWAS | TFC | INIT | NORMALIZATION: " +
            Object.keys(nnTools.getNormalization()) +
            "\n==============================="
        )
      );
      break;
    ...
    case "USER_AUTHENTICATED":
      ...
      if (twitterUserObj !== undefined) {
        ...
        twitterUserObj.twitterConfig.access_token_secret =
          authObjNew.access_token_secret;
        twitterUserObj.twitterConfig.TOKEN = authObjNew.access_token;
        twitterUserObj.twitterConfig.TOKEN_SECRET =
          authObjNew.access_token_secret;
    ...
    case "USER_CATEGORIZE":
        ...
        console.log(
          chalkInfo(
            MODULE_ID_PREFIX +
              ...
              m.user.screenName +
              " | CN: " +
              m.user.categorizeNetwork +
              " | CV: " +
              m.user.categoryVerified +
              " | CM: " +
              m.user.category +
              " | CA: " +
              m.user.categoryAuto
      ...
      if (
        m.priorityFlag ||
        (cacheObj === undefined &&
          processUserQueue.length < USER_PROCESS_QUEUE_MAX_LENGTH)
      ) {
        try {
    ...
    default:
      console.error(
        chalkLog(
          MODULE_ID_PREFIX + " | TWP | *** UNKNOWN OP" + " | INTERVAL: " + m.op
        )
      );
  }
});
setTimeout(async function () {
  try {
    ...
    try {
      ...
      console.log(
        chalkError(
          MODULE_ID_PREFIX +
            " | *** INIT ERROR | CONFIG FILE NOT FOUND? | ERROR: " +
            err
        )
      );
    }
  ...
  try {
    ...
    console.log(
      chalkError(MODULE_ID_PREFIX + " | *** INIT INFO TWITTER ERROR: " + err)
    );
  }
}, ONE_SECOND);
