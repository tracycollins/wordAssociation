...
function dnsReverse(params){
  ...
  return new Promise(function(resolve, reject){
    ...
    if (ipCacheObj) {
      ...
      ipCacheObj.timeStamp = tcUtils.getTimeStamp();
...
function quit(message) {
  ...
  console.log(chalkAlert("\nWAS | ... QUITTING ... " + tcUtils.getTimeStamp()));
...
function printHashtag(params) {
  ...
  if (params.verbose) {
    return tcUtils.jsonPrint(params.hashtag);
  } 
  else {
    text = "#" + hashtag.nodeId
    + " | M  " + hashtag.mentions
    + " | LS " + tcUtils.getTimeStamp(hashtag.lastSeen)
          ...
          userServerController.findOneUser(user, {noInc: true, fields: fieldsExclude}, async function(err, updatedUser){
            ...
            if (configuration.verbose) {
              console.log(chalk.blue(MODULE_ID_PREFIX + " | UPDATED updatedUser"
                ...
                + " | USER CR: " + tcUtils.getTimeStamp(updatedUser.createdAt)
...
function viewerCacheExpired(viewerCacheId, viewerObj) {
  ...
  console.log(chalkInfo(MODULE_ID_PREFIX + " | XXX VIEWER CACHE EXPIRED"
    ...
    + "\nNOW: " + tcUtils.getTimeStamp()
    + " | TS: " + tcUtils.getTimeStamp(viewerObj.timeStamp)
...
function serverCacheExpired(serverCacheId, serverObj) {
  ...
  console.log(chalkInfo(MODULE_ID_PREFIX + " | XXX SERVER CACHE EXPIRED"
    ...
    + "\nNOW: " + tcUtils.getTimeStamp()
    + " | TS: " + tcUtils.getTimeStamp(serverObj.timeStamp)
...
function authenticatedSocketCacheExpired(socketId, authSocketObj) {
  ...
  console.log(chalkInfo(MODULE_ID_PREFIX + " | XXX AUTH SOCKET CACHE EXPIRED"
    ...
    + " | NOW: " + tcUtils.getTimeStamp()
    + " | TS: " + tcUtils.getTimeStamp(authSocketObj.timeStamp)
      ...
      socketIds.forEach(function(socketId){
        ...
        if (authSocketObjCache !== undefined) {
          ...
          console.log(chalkInfo(MODULE_ID_PREFIX + " | AUTH SOCKET CACHE ENTRIES"
            ...
            + " | NOW: " + tcUtils.getTimeStamp()
            + " | TS: " + tcUtils.getTimeStamp(authSocketObjCache.timeStamp)
...
function showStats(options){
  ...
  console.log(chalkLog(MODULE_ID_PREFIX + " | S"
    + " | " + tcUtils.getTimeStamp()
    + " | E: " + statsObj.elapsed
    + " | S: " + tcUtils.getTimeStamp(parseInt(statsObj.startTime))
...
async function loadMaxInputHashMap(p){
  ...
  try{
    ...
    if (empty(dataObj.maxInputHashMap)) {
      console.log(chalkError(MODULE_ID_PREFIX + " | *** ERROR: loadMaxInputHashMap: loadFile: maxInputHashMap UNDEFINED"));
    }
    if (empty(dataObj.normalization)) {
      console.log(chalkError(MODULE_ID_PREFIX + " | *** ERROR: loadMaxInputHashMap: loadFile: normalization UNDEFINED"));
    }
    ...
    if (err.code == "ENOTFOUND") {
      console.log(chalkError(MODULE_ID_PREFIX + " | *** LOAD MAX INPUT: FILE NOT FOUND"
...
configEvents.on("CHILD_ERROR", function childError(childObj){
  ...
  console.log(chalkError(MODULE_ID_PREFIX + " | *** CHILD_ERROR"
    + " | " + childObj.childId
    + " | ERROR: " + tcUtils.jsonPrint(childObj.err)
  ));
  ...
  switch(childObj.childId){
    ...
    case DEFAULT_TFE_CHILD_ID:
      ...
      console.log(chalkError(MODULE_ID_PREFIX + " | *** KILL TFE CHILD"));
...
configEvents.on("INTERNET_READY", function internetReady() {
  ...
  console.log(chalkInfo(tcUtils.getTimeStamp() + " | SERVER_READY EVENT"));
  ...
  if (!httpServer.listening) {
    ...
    httpServer.on("reconnect", function serverReconnect() {
      ...
      debug(chalkConnect(tcUtils.getTimeStamp() + " | PORT RECONNECT: " + config.port));
    });

    httpServer.on("connect", function serverConnect() {
      statsObj.socket.connects += 1;
      statsObj.internetReady = true;
      debug(chalkConnect(tcUtils.getTimeStamp() + " | PORT CONNECT: " + config.port));

      httpServer.on("disconnect", function serverDisconnect() {
        statsObj.internetReady = false;
        console.log(chalkError(MODULE_ID_PREFIX + " | *** PORT DISCONNECTED | " + tcUtils.getTimeStamp() 
          + " | " + config.port));
      });
    });

    httpServer.listen(config.port, function serverListen() {
      debug(chalkInfo(MODULE_ID_PREFIX + " | " + tcUtils.getTimeStamp() + " | LISTENING ON PORT " + config.port));
    });
    ...
    httpServer.on("error", function serverError(err) {
      ...
      debug(chalkError(MODULE_ID_PREFIX + " | *** HTTP ERROR | " + tcUtils.getTimeStamp() + "\n" + err));
...
configEvents.on("NEW_BEST_NETWORK", function configEventDbConnect(){
  ...
  if (tfeChild !== undefined) {
    ...
    console.log(chalkBlue(MODULE_ID_PREFIX + " | >>> UPDATE TFE CHILD NETWORK"
      + " | " + tcUtils.getTimeStamp()
    ...
    tfeChild.send({ op: "NETWORK", networkObj: bestNetworkObj }, function tfeNetwork(err){
      if (err) {
        console.log(chalkError(MODULE_ID_PREFIX + " | *** TFE CHILD SEND NETWORK ERROR"
          + " | " + err
        ));
      }
    });
...
configEvents.on("NEW_MAX_INPUT_HASHMAP", function configEventDbConnect(){
  ...
  if (tfeChild !== undefined) {
    ...
    console.log(chalkBlue(MODULE_ID_PREFIX + " | UPDATE TFE CHILD MAX INPUT HASHMAP: " + Object.keys(maxInputHashMap)));

    tfeChild.send(
      { op: "MAX_INPUT_HASHMAP", maxInputHashMap: maxInputHashMap }, 
      function tfeMaxInputHashMap(err){
      if (err) {
        console.log(chalkError(MODULE_ID_PREFIX + " | *** TFE CHILD SEND MAX_INPUT_HASHMAP ERROR"
          + " | " + err
        ));
      }
    });
      ...
      for(const sub of bodyJson){
        ...
        if ((aaSubs.application_id == "15917082") && (aaSubs.subscriptions.length > 0)){
          ...
          console.log(chalkTwitter(MODULE_ID_PREFIX + " | +++ TWITTER ACCOUNT ACTIVITY SUBSCRIPTIONS"
            + "\n" + tcUtils.jsonPrint(statsObj.twitter.aaSubs)
          ));
        }
        else {
...
async function addTwitterAccountActivitySubscription(p){
  ...
  if (!threeceeTwitter) {
    ...
    console.log("threeceeTwitter\n" + tcUtils.jsonPrint(threeceeTwitter));
...
function socketRxTweet(tw) {
  ...
  if (tweetRxQueue.length > maxQueue){
    ...
    if (saveSampleTweetFlag) {
      ...
      const sampleTweetFileName = "sampleTweet_" + tcUtils.getTimeStamp() + ".json";
...
async function initUncatUserCache(){
  ...
  try{
    ...
    for(const userId of uncatUserIdArray){
      uncatUserCacheObj[userId].timeStamp = tcUtils.getTimeStamp();
      uncatUserCache.set(
        userId, 
    ...
    console.log(chalkLog(MODULE_ID_PREFIX + " | +++ LOADED UNCAT USER CACHE FILE"
      ...
      + " | " + folder + "/" + uncatUserCacheFile
      + " | $ EXPIRED: " + statsObj.caches.uncatUserCache.expired
      + "\nUNCAT USER $ STATS\n" + tcUtils.jsonPrint(uncatUserCache.getStats())
    ));
...
async function initSocketHandler(socketObj) {
  ...
  try{
    ...
    socket.on("reconnect_error", function reconnectError(errorObj) {
      ...
      console.log(chalkError(tcUtils.getTimeStamp(timeStamp) 
        + " | SOCKET RECONNECT ERROR: " + socketId 
        + "\nerrorObj\n" + tcUtils.jsonPrint(errorObj)
      ));
    });
    ...
    socket.on("error", function socketError(error) {
      ...
      if (currentServer !== undefined) { 
        ...
        console.log(chalkError(MODULE_ID_PREFIX + " | SERVER ERROR" 
          + " | " + tcUtils.getTimeStamp(currentServer.timeStamp)
      ...
      if (currentViewer !== undefined) { 
        ...
        console.log(chalkError(MODULE_ID_PREFIX + " | VIEWER ERROR" 
          + " | " + tcUtils.getTimeStamp(currentViewer.timeStamp)
    ...
    socket.on("reconnect", function socketReconnect() {
      ...
      console.log(chalkConnect(tcUtils.getTimeStamp(timeStamp) + " | SOCKET RECONNECT: " + socket.id));
    });
    ...
    socket.on("SESSION_KEEPALIVE", function sessionKeepalive(keepAliveObj) {
      ...
      if (empty(keepAliveObj.user)) {
        console.log(chalkAlert(MODULE_ID_PREFIX + " | SESSION_KEEPALIVE USER UNDEFINED ??"
          ...
          + " | " + socket.id
          + " | " + ipAddress
          + "\n" + tcUtils.jsonPrint(keepAliveObj)
        ));
        return;
      }
      ...
      switch (currentSessionType) {
        ...
        default:
          console.log(chalkAlert(MODULE_ID_PREFIX + " | **** NOT SERVER ****"
            ...
            + "\n" + tcUtils.jsonPrint(keepAliveObj.user)
          ));
      }
    });
    ...
    socket.on("USER_READY", function userReady(userObj) {
      ...
      console.log(chalkSocket("R< USER READY"
        ...
        + " | " + socket.id
        + " | " + userObj.userId
        + " | SENT " + tcUtils.getTimeStamp(parseInt(userObj.timeStamp))
      ));
    ...
    socket.on("VIEWER_READY", async function viewerReady(viewerObj) {
      ...
      console.log(chalkSocket("VIEWER READY"
        ...
        + " | " + socket.id
        + " | " + viewerObj.viewerId
        + " | SENT AT " + tcUtils.getTimeStamp(parseInt(viewerObj.timeStamp))
      ));
...
async function initSocketNamespaces(){
  ...
  try {
    ...
    console.log(chalkInfo(tcUtils.getTimeStamp(timeStamp) + " | INIT SOCKET NAMESPACES"));
    ...
    adminNameSpace.on("connect", function adminConnect(socket) {
      ...
      if (authenticatedSocketObj !== undefined){
        console.log(chalkAlert(MODULE_ID_PREFIX + " | ADMIN ALREADY AUTHENTICATED"
          ...
          + "\n" + tcUtils.jsonPrint(authenticatedSocketObj)
        ));
      }
      else {
...
function logHeartbeat() {
  ...
  console.log(chalkLog("HB " + heartbeatsSent 
    + " | " + tcUtils.getTimeStamp() 
    + " | ST: " + tcUtils.getTimeStamp(parseInt(statsObj.startTime)) 
...
function initAppRouting(callback) {
  ...
  console.log(chalkInfo(tcUtils.getTimeStamp() + " | INIT APP ROUTING"));
  ...
  app.use(async function requestLog(req, res, next) {
    ...
    if (req.path == "/json") {
      ...
      console.log(chalkAlert(MODULE_ID_PREFIX + " | R< callbacks/addsub"
        + "\nreq.query\n" + tcUtils.jsonPrint(req.query)
        + "\nreq.params\n" + tcUtils.jsonPrint(req.params)
        + "\nreq.body\n" + tcUtils.jsonPrint(req.body)
      )); 
    }
      ...
      if (req.method == "GET") {
        ...
        if (followEvents && (followEvents[0].type == "unfollow")) {
          ...
          console.log(chalkAlert(MODULE_ID_PREFIX + " | >>> TWITTER USER UNFOLLOW EVENT"
            + " | SOURCE: @" + followEvents[0].source.screen_name
            + " | TARGET: @" + followEvents[0].target.screen_name
            // + "\n" + tcUtils.jsonPrint(followEvents)
          ));
      ...
      fs.readFile(fullPath, "utf8", function(err, data) {
        ...
        if (err) {
          ...
          console.log(chalkInfo(tcUtils.getTimeStamp()
  ...
  app.get("/account", ensureAuthenticated, async function(req, res){
    ...
    userServerController.findOne({ user: req.session.passport.user}, function(err, user) {
      if(err) {
        console.log(chalkError(MODULE_ID_PREFIX + " | *** ERROR TWITTER AUTHENTICATION: " + tcUtils.jsonPrint(err))); // handle errors
...
function testInternetConnection(params, callback) {
  ...
  testClient.on("connect", function testConnect() {
    ...
    console.log(chalkInfo(tcUtils.getTimeStamp() + " | CONNECTED TO " + params.url + ": OK"));
    console.log(chalkInfo(tcUtils.getTimeStamp() + " | SEND INTERNET_READY"));
  ...
  testClient.on("error", function testError(err) {
    ...
    if (err) {
      if (err.code != "ENOTFOUND") {
        console.log(chalkError(MODULE_ID_PREFIX + " | testClient ERROR " + tcUtils.jsonPrint(err)));
      }
    }
    ...
    console.log(chalkError(tcUtils.getTimeStamp()
...
function initInternetCheckInterval(interval){
  ...
  return new Promise(function(resolve){
    ...
    // debug(chalkInfo(tcUtils.getTimeStamp() 
    ...
    dbuPingInterval = setInterval(function(){
      ...
      if (!dbuPingSent) {
        ...
        dbuChild.send({op: "PING", pingId: dbuPingId}, function(err){
          ...
          console.log(chalkInfo(MODULE_ID_PREFIX + " | >PING | DBU | PING ID: " + tcUtils.getTimeStamp(dbuPingId)));
        ...
        dbuChild.send({op: "PING", pingId: dbuPingId}, function(err){
          ...
          if (configuration.verbose) { console.log(chalkInfo(MODULE_ID_PREFIX + " | >PING | DBU | PING ID: " + tcUtils.getTimeStamp(dbuPingId))); }
        ...
        console.log(chalkAlert(MODULE_ID_PREFIX + " | *** PONG TIMEOUT | DBU"
          + " | TIMEOUT: " + interval
          + " | NOW: " + tcUtils.getTimeStamp()
          + " | PING ID: " + tcUtils.getTimeStamp(dbuPingId)
      ...
      if (!tfePingSent) {
        ...
        tfeChild.send({op: "PING", pingId: tfePingId}, function(err){
          ...
          if (err) {
            ...
            console.log(chalkError(MODULE_ID_PREFIX + " | *** TFE SEND PING ERROR: " + err));
          ...
          console.log(chalkInfo(MODULE_ID_PREFIX + " | >PING | TFE | PING ID: " + tcUtils.getTimeStamp(tfePingId)));
        ...
        tfeChild.send({op: "PING", pingId: tfePingId}, function(err){
          ...
          if (configuration.verbose) { console.log(chalkInfo(MODULE_ID_PREFIX + " | >PING | TFE | PING ID: " + tcUtils.getTimeStamp(tfePingId))); }
        ...
        console.log(chalkAlert(MODULE_ID_PREFIX + " | *** PONG TIMEOUT | TFE"
          + " | TIMEOUT: " + interval
          + " | NOW: " + tcUtils.getTimeStamp()
          + " | PING ID: " + tcUtils.getTimeStamp(tfePingId)
          + " | ELAPSED: " + tcUtils.msToTime(moment().valueOf() - tfePingId)
        ));
    ...
    tssPingInterval = setInterval(function(){
      ...
      if (!tssPingSent) {
        ...
        tssChild.send({op: "PING", pingId: tssPingId}, function(err){
          ...
          console.log(chalkInfo(MODULE_ID_PREFIX + " | >PING | TSS | PING ID: " + tcUtils.getTimeStamp(tssPingId)));
        ...
        tssChild.send({op: "PING", pingId: tssPingId}, function(err){
          ...
          if (configuration.verbose) { console.log(chalkInfo(MODULE_ID_PREFIX + " | >PING | TSS | PING ID: " + tcUtils.getTimeStamp(tssPingId))); }
        ...
        console.log(chalkAlert(MODULE_ID_PREFIX + " | *** PONG TIMEOUT | TSS"
          + " | TIMEOUT: " + interval
          + " | NOW: " + tcUtils.getTimeStamp()
          + " | PING ID: " + tcUtils.getTimeStamp(tssPingId)
      ...
      switch (m.op) {
        ...
        case "FOLLOW_LIMIT":
          ...
          console.log(chalkInfo(MODULE_ID_PREFIX + " | <TSS | FOLLOW LIMIT"
            + " | 3C @" + m.threeceeUser
            + " | LIMIT: " + tcUtils.getTimeStamp(m.twitterFollowLimit)
            + " | NOW: " + tcUtils.getTimeStamp()
          ));
        ...
        case "PONG":
          ...
          if (configuration.verbose) {
            console.log(chalkInfo(MODULE_ID_PREFIX + " | <TSS | PONG"
              + " | NOW: " + tcUtils.getTimeStamp()
              + " | PONG ID: " + tcUtils.getTimeStamp(m.pongId)
...
async function initTfeChild(params){
  ...
  console.log(chalk.bold.black(MODULE_ID_PREFIX + " | INIT TFE CHILD\n" + tcUtils.jsonPrint(params)));

  console.log(chalkInfo(MODULE_ID_PREFIX + " | LOADED MAX INPUT HASHMAP + NORMALIZATION"));
  console.log(chalkInfo(MODULE_ID_PREFIX + " | MAX INPUT HASHMAP INPUT TYPES: " + Object.keys(maxInputHashMap)));
  console.log(chalkInfo(MODULE_ID_PREFIX + " | NORMALIZATION INPUT TYPES: " + Object.keys(normalization)));
  ...
  tfe.on("message", async function tfeMessageRx(m){
    ...
    switch (m.op) {
      ...
      case "ERROR":
        ...
        console.log(chalkError(MODULE_ID_PREFIX + " | <TFE | ERROR"
          + " | 3C @" + m.threeceeUser
          + " | INFO 3C: " + isInfoUser
          + " | ERROR TYPE: " + m.errorType
        ));
      ...
      case "TWITTER_STATS":
        ...
        console.log(chalkInfo(MODULE_ID_PREFIX + " | <TFE | TWITTER STATS"
          + " | 3C @" + m.threeceeUser
          + " | FOLLOWING: " + m.twitterFollowing
        ));
      ...
      case "FOLLOW_LIMIT":
        ...
        console.log(chalkInfo(MODULE_ID_PREFIX + " | <TFE | FOLLOW LIMIT"
          + " | 3C @" + m.threeceeUser
          + " | LIMIT: " + tcUtils.getTimeStamp(m.twitterFollowLimit)
          + " | NOW: " + tcUtils.getTimeStamp()
        ));
      ...
      case "USER_CATEGORIZED":
        ...
        if (m.priorityFlag){
          if (m.searchMode === "SPECIFIC"){
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | MODE: " + m.searchMode, m.user);
            ...
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | MISMATCH NOT FOUND | " + m.searchMode, m.user);
            await twitterSearchUser({searchNode: "@?mm"});
          }
          else if ((m.searchMode === "UNCAT_LEFT") && m.user.categoryAuto && (m.user.categoryAuto !== "left")){
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | UNCAT LEFT NOT FOUND | " + m.searchMode, m.user);
            ...
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | UNCAT NEUTRAL NOT FOUND | " + m.searchMode, m.user);
            await twitterSearchUser({searchNode: "@?neutral"});
          }
          else if ((m.searchMode === "UNCAT_RIGHT") && m.user.categoryAuto && (m.user.categoryAuto !== "right")){
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | UNCAT RIGHT NOT FOUND | " + m.searchMode, m.user);
            await twitterSearchUser({searchNode: "@?right"});
          }
          else if ((m.searchMode === "UNCAT") && m.user.category){
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | UNCAT NOT FOUND | " + m.searchMode, m.user);
            ...
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | AUTO FOLLOW", m.user);
            // autoFollowUserSet.delete(m.user.nodeId);
          }
        }
        else if (configuration.verbose) {
          printUserObj(MODULE_ID_PREFIX + " | <TFE | CAT | MODE: " + m.searchMode, m.user, chalkLog);
        }
      break;
      ...
      case "USER_CATEGORIZED_ERROR":
        if (m.priorityFlag){
          ...
          if (m.searchMode === "MISMATCH"){
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | *** MISMATCH ERROR | " + m.searchMode, m.user);
            await twitterSearchUser({searchNode: "@?mm"});
          }
          else if (m.searchMode === "UNCAT_LEFT"){
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | *** UNCAT LEFT ERROR | " + m.searchMode, m.user);
            await twitterSearchUser({searchNode: "@?left"});
          }
          else if (m.searchMode === "UNCAT_NEUTRAL"){
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | *** UNCAT NEUTRAL ERROR | " + m.searchMode, m.user);
            await twitterSearchUser({searchNode: "@?neutral"});
          }
          else if (m.searchMode === "UNCAT_RIGHT"){
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | *** UNCAT RIGHT ERROR | " + m.searchMode, m.user);
            await twitterSearchUser({searchNode: "@?right"});
          }
          else{
            printUserObj(MODULE_ID_PREFIX + " | <TFE | PRI CAT | *** UNCAT ERROR | " + m.searchMode, m.user);
            await twitterSearchUser({searchNode: "@?all"});
          }
        }
        else if (configuration.verbose) {
      ...
      case "PONG":
        ...
        childrenHashMap[params.childId].status = "RUNNING";
        if (configuration.verbose) {
          console.log(chalkInfo(MODULE_ID_PREFIX + " | <TFE | PONG"
            + " | NOW: " + tcUtils.getTimeStamp()
            + " | PONG ID: " + tcUtils.getTimeStamp(m.pongId)
            + " | RESPONSE TIME: " + tcUtils.msToTime(moment().valueOf() - m.pongId)
          ));
      ...
      default:
        console.log(chalkError(MODULE_ID_PREFIX + " | TFE | *** ERROR *** UNKNOWN OP: " + m.op));
    }
  });
    ...
    tweetParserPingInterval = setInterval(function(){
      ...
      if (!tweetParserPingSent) {
        ...
        twpChild.send({op: "PING", pingId: tweetParserPingId}, function(err){
          ...
          if (configuration.verbose) { console.log(chalkInfo(MODULE_ID_PREFIX + " | >PING | TWEET_PARSER | PING ID: " + tcUtils.getTimeStamp(tweetParserPingId))); }
        ...
        console.log(chalkAlert(MODULE_ID_PREFIX + " | *** PONG TIMEOUT | TWEET_PARSER"
          + " | TIMEOUT: " + interval
          + " | NOW: " + tcUtils.getTimeStamp()
          + " | PING ID: " + tcUtils.getTimeStamp(tweetParserPingId)
...
async function initThreeceeTwitterUser(threeceeUser){
  ...
  try {
    ...
    //   + "\nCONFIG\n" + tcUtils.jsonPrint(threeceeTwitter.twitterConfig)
    // ));
...
async function twitterGetUserUpdateDb(params){
  ...
  try{
    if (configuration.verbose) {
      printUserObj(MODULE_ID_PREFIX + " | GET USER TWITTER DATA", user);
    }
    ...
    if (configuration.verbose) {
      printUserObj(MODULE_ID_PREFIX + " | FOUND users/show rawUser", cUser);
    }
    ...
    console.log(chalkError(MODULE_ID_PREFIX + " | ***ERROR twitterGetUserUpdateDb"
      + "\n" + printUser({user: user})
    ));
    throw err;
  }
...
async function processTwitterSearchNode(params) {
  ...
  if (params.user) {
    ...
    if (params.specificUserFlag) {
      ...
      uncatUserObj.timeStamp = tcUtils.getTimeStamp();
      ...
      if (!empty(params.user.category) && (params.user.category != params.user.categoryAuto)) {
        ...
        mismatchUserObj.timeStamp = tcUtils.getTimeStamp();
        ...
        console.log(chalk.blue(MODULE_ID_PREFIX
          ...
          + " | CAT A: " + params.user.categoryAuto
          + " | $ EXPIRED: " + statsObj.caches.mismatchUserCache.expired
          + "\nMISMATCH USER $ STATS\n" + tcUtils.jsonPrint(mismatchUserCache.getStats())
        ));
      }
      ...
      uuObj.timeStamp = tcUtils.getTimeStamp();
            ...
            switch (searchMode){
              ...
              case "UNCAT":
                if (user.category && (user.category != "none")){
                  userSkipCount += 1;
                  printUserObj(MODULE_ID_PREFIX + " | ... SKIP SEARCH | MODE: " + searchMode + " | SKIP COUNT: " + userSkipCount, user);
                  notFoundAndMore = true;
                  break;
                }

                printUserObj(MODULE_ID_PREFIX + " | --> UNCAT USER", user);
              ...
              case "MISMATCH":
                if ((user.category !== undefined) && (user.category != "none") && user.categoryVerified){
                  userSkipCount += 1;
                  printUserObj(MODULE_ID_PREFIX + " | ... SKIP SRCH | VRFD | MODE: " + searchMode + " | SKIPPED: " + userSkipCount, user);
                  notFoundAndMore = true;
                  break;
                }
                else if ((user.category !== undefined) && (user.category != "none") && (user.category == user.categoryAuto)){
                  printUserObj(MODULE_ID_PREFIX + " | ... SKIP SRCH | MTCHD  | MODE: " + searchMode + " | SKIPPED: " + userSkipCount, user);
                  notFoundAndMore = true;
                  break;
                }

                printUserObj(MODULE_ID_PREFIX + " | --> MM USER", user);
              ...
              case "UNCAT_LEFT":
                ...
                if ((user.category && (user.category != "none")) || (user.categoryAuto != "left")){
                  userSkipCount += 1;
                  printUserObj(MODULE_ID_PREFIX + " | ... SKIP SEARCH USER | MODE: " + searchMode + " | SKIP COUNT: " + userSkipCount, user);
                  notFoundAndMore = true;
                  break;
                }

                printUserObj(MODULE_ID_PREFIX + " | --> SEARCH USER FOUND | MODE: " + searchMode + " | SKIP COUNT: " + userSkipCount, user);
...
async function twitterSearchUser(params) {
  ...
  console.log(chalkInfo(MODULE_ID_PREFIX + " | SEARCH FOR SPECIFIC USER | @" + searchNodeUser.screenName));
    ...
    dbUserMissQueueInterval = setInterval(async function() {
      ...
      if (dbUserMissQueueReady && (dbUserMissQueue.length > 0)) {
        ...
        try {
          ...
          console.log(chalkError(MODULE_ID_PREFIX + " | *** DB MISS USER SEARCH ERROR: " + err));
          dbUserMissQueueReady = true;
        }
...
function waitDbConnectionReady(){
  ...
  return new Promise(function(resolve){
    ...
    dbConnectionReadyInterval = setInterval(function(){
      ...
      console.log(chalkBlue(MODULE_ID_PREFIX + " | ... WAIT DB CONNECTION | " + tcUtils.getTimeStamp() ));

      if (statsObj.dbConnectionReady) {
        console.log(chalk.green(MODULE_ID_PREFIX + " | +++ DB CONNECTION | " + tcUtils.getTimeStamp() ));
...
async function initWatchConfig(){
  ...
  const loadConfig = async function(f){
    ...
    try{
      ...
      if (botBlockListFileRegex.test(f)){
        console.log(chalkAlert(MODULE_ID_PREFIX + " | BOT BLOCK FILE CHANGED | " + tcUtils.getTimeStamp() + " | " + f));
  ...
  watch.createMonitor(configHostFolder, watchOptions, function (monitor) {
    ...
    monitor.on("removed", function (f) {
      console.log(chalkAlert(MODULE_ID_PREFIX + " | XXX FILE DELETED | " + tcUtils.getTimeStamp() + " | " + f));
    });
  });
...
setTimeout(async function(){
  ...
  try {
    ...
    console.log(MODULE_ID_PREFIX + " | " + chalkTwitter(configuration.processName + " STARTED " + tcUtils.getTimeStamp() ));
