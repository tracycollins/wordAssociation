...
const DEFAULT_MIN_FOLLOWERS_AUTO_CATEGORIZE = 4000;
  ...
  try {
    ...
    socket.on("TWITTER_CATEGORY_VERIFIED", async function (user) {
      ...
      try {
        ...
        if (node && node.nodeType === "user") {
          socket.emit("UPDATE_TWITTER_USER", { node: node, stats: statsObj });
        }
      } catch (err) {
        console.log(
    ...
    socket.on("TWITTER_IGNORE", async function (user) {
      ...
      try {
        ...
        socket.emit("UPDATE_TWITTER_USER", { node: user, stats: statsObj });
      } catch (err) {
        ...
        try {
          ...
          if (node) {
            if (node.nodeType === "user") {
              ...
              viewNameSpace.emit("UPDATE_TWITTER_USER", {
                node: node,
                stats: statsObj,
              });
              updateUserCounts();
