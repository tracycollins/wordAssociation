...
function initTxNodeGeneratorInterval(interval){

  console.log(chalkLog("INIT TX NODE GENERATOR INTERVAL | " + interval + " MS"));

  clearInterval(txNodeGeneratorInterval);

  txNodeGeneratorInterval = setInterval(function txNodeGenerator() {

    const text = randomItem(hashtagArray);

    const randomNode = new Hashtag({ 
      nodeType: "hashtag",
      isKeyword: true,
      nodeId : text,
      hashtagId : text,
    ...
    if (transmitNodeQueue.size() < MAX_Q) {
      transmitNodeQueue.enqueue(randomNode);

      console.log(chalkLog("TNG [" + transmitNodeQueue.size() + "] " + randomNode.nodeId));
...
initialize(configuration, function initializeComplete(err) {
  if (err) {
    ...
    initTxNodeGeneratorInterval(100);
