...
function socketRxTweet(tw) {
  ...
  if (tweetRxQueue.length > maxTweetRxQueue) {
    ...
    for(let i=0; i<tw.includes.users.length; i++){
      if (categorizedUserHashMap.has(tw.includes.users[i].username.toLowerCase())) {
        tw.includes.users[i].following = true;
        tw.includes.users[i].categorized = true;
        tw.includes.users[i].category = categorizedUserHashMap.get(tw.user.id_str).manual;
        tw.includes.users[i].categoryAuto = categorizedUserHashMap.get(tw.user.id_str).auto;
        tw.includes.users[i].categorizeNetwork = categorizedUserHashMap.get(
          tw.includes.users[i].id
        ).network;
        tw.includes.users[i].categoryVerified = categorizedUserHashMap.get(
          tw.includes.users[i].id
        ).verified;
      }
    }
